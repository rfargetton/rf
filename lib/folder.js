...
export async function getAllFilesId(folderName){
  ...
  const localeDirs = fs.readdirSync(fullDir, { withFileTypes: true}).filter(dirent => dirent.isDirectory()).map(dirent => dirent.name);
  const allFiles = localeDirs.map(dir => ({locale: dir, files: fs.readdirSync(path.join(contentDir, folderName, dir))}) );

  return allFiles.map(locale => locale.files.map(file => ({params: {id: file.replace(/\.md$/, "")}, locale: locale.locale}))).flat();
